...
final class CaptureServiceTests: XCTestCase {
    func testStopMonitoringWithoutStartDoesNotRequireAnEngine() {
        let staging = FileManager.default.temporaryDirectory
            .appendingPathComponent("setcatcher-capture-idle-\(UUID().uuidString)", isDirectory: true)
    ...
    func testStartMonitoringUnknownDeviceThrowsDeviceMissing() throws {
        ...
        let staging = FileManager.default.temporaryDirectory
            .appendingPathComponent("setcatcher-capture-missing-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: staging) }
        let service = CaptureService(stagingDirectory: staging)
        let ghost = AudioInputDevice(
            id: "setcatcher-missing-uid-\(UUID().uuidString)",
            name: "Ghost Input",
            manufacturer: "None"
        )
    ...
    func testLivePioneerInputChannelPeaks() async throws {
        guard ProcessInfo.processInfo.environment["SETCATCHER_LIVE_XZ"] == "1" else {
            throw XCTSkip("Set SETCATCHER_LIVE_XZ=1 for the hardware bench.")
        }
        guard CaptureService.microphonePermissionGranted() else {
    ...
    func testLivePioneerInputRecords16Bit48kCapture() async throws {
        ...
        let staging = FileManager.default.temporaryDirectory
            .appendingPathComponent("setcatcher-live-xz-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: staging) }
        ...
        let resultPath = ProcessInfo.processInfo.environment["SETCATCHER_LIVE_HARDWARE_RESULT_PATH"]
            ?? "/tmp/setcatcher-live-xz-result.txt"
    ...
    func testLiveUserArchiveLinkerGroupsCaptureWithFolder() throws {
        ...
        let group = try XCTUnwrap(
            groups.first { group in
                group.primary.originalFilename.contains("SetCatcher-xz-bench")
    ...
    func testLiveDiskFullOnTinyVolumeThrowsDiskFull() throws {
        ...
        guard FileManager.default.fileExists(atPath: volume.path, isDirectory: &isDir), isDir.boolValue else {
            throw XCTSkip("Attach /tmp/setcatcher-diskfull.dmg first.")
        }
