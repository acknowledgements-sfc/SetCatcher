...
final class AudioInputDeviceCatalogTests: XCTestCase {
    ...
    func testUnknownUIDDoesNotResolveToAudioDeviceID() {
        ...
        XCTAssertNil(AudioInputDeviceCatalog.audioDeviceID(forUID: "setcatcher-missing-uid-\(UUID().uuidString)"))
        XCTAssertEqual(AudioInputDeviceCatalog.inputChannelCount(forUID: ""), 0)
        XCTAssertFalse(AudioInputDeviceCatalog.isSupportedCaptureFormat(forUID: "setcatcher-missing-uid-\(UUID().uuidString)"))
    }
