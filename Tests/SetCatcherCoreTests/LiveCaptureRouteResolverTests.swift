...
final class LiveCaptureRouteResolverTests: XCTestCase {
    ...
    func testVerifiedHardwareFeedBeatsDriver() {
        let facts = LiveCaptureRouteFacts(
            ...
            driverAvailability: .available(deviceID: "setcatcher-audio"),
...
final class SetCatcherAudioDriverIdentityTests: XCTestCase {
    func testUsesSetCatcherDisplayIdentityAndKeepsLegacyNameHint() {
        XCTAssertEqual(SetCatcherAudioDriverIdentity.deviceName, "SetCatcher Audio")
        XCTAssertEqual(SetCatcherAudioDriverIdentity.manufacturer, "SetCatcher")
        XCTAssertTrue(SetCatcherAudioDriverIdentity.deviceNameHints.contains("SetCatcher Audio"))
    }

    func testMatchesByDeviceUID() {
        // AudioInputDevice.id is the Core Audio device UID.
        let driver = AudioInputDevice(
            id: SetCatcherAudioDriverIdentity.deviceUID,
            name: "Renamed By User",
            manufacturer: "SetCatcher",
            transportType: .virtual
        )
        XCTAssertTrue(SetCatcherAudioDriverIdentity.matchesByUID(driver))
        XCTAssertTrue(SetCatcherAudioDriverIdentity.matches(driver))
        XCTAssertEqual(
            SetCatcherAudioDriverIdentity.availability(in: [driver]),
            .available(deviceID: SetCatcherAudioDriverIdentity.deviceUID)
        )
        XCTAssertEqual(
            SetCatcherAudioDriverClient.recordingBackend(availability: .available(deviceID: "x")),
            .setcatcherDriver
        )
    }
    ...
    func testNameMatchIsSecondaryAndUIDMatchIsExact() {
        ...
        XCTAssertTrue(SetCatcherAudioDriverIdentity.matches(namedOnly), "name is accepted as a secondary signal")
        XCTAssertFalse(SetCatcherAudioDriverIdentity.matchesByUID(namedOnly), "but it is not identity")

        let legacyName = AudioInputDevice(id: "legacy-uid", name: "SetCatcher Audio", manufacturer: "SetCatcher", transportType: .virtual)
        XCTAssertTrue(SetCatcherAudioDriverIdentity.matches(legacyName), "legacy display name remains discoverable")

        let impostor = AudioInputDevice(id: "agg", name: "SetCatcher Audio Aggregate", manufacturer: "Someone Else", transportType: .virtual)
        XCTAssertFalse(SetCatcherAudioDriverIdentity.matches(impostor), "substring names must not match")
    }

    func testDoesNotMatchUSBOrWrongName() {
        let usb = AudioInputDevice(id: "u", name: "SetCatcher Audio", manufacturer: "SetCatcher", transportType: .usb)
        let other = AudioInputDevice(id: "sva", name: "Serato Virtual Audio", manufacturer: "Serato", transportType: .virtual)
        XCTAssertFalse(SetCatcherAudioDriverIdentity.matches(usb))
        XCTAssertFalse(SetCatcherAudioDriverIdentity.matches(other))
        XCTAssertEqual(SetCatcherAudioDriverIdentity.availability(in: [usb, other]), .missing)
    }
}
...
final class DJAppOutputRoutingTests: XCTestCase {
    func testSeratoAdapterDetectsRunningAppAndIsNotAutomated() {
        ...
        XCTAssertEqual(adapter.applyRouteToSetCatcherDriver(), .notAutomatedYet)
