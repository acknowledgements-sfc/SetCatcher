...
> **Reconciling the matrix with the dated evidence below.** The 2026-08-12 five-app probe recorded `PASS meter+write` for all five (Serato, rekordbox, djay Pro 2, VirtualDJ, Traktor DJ 2), and the 2026-08-14 run confirmed Serato/rekordbox Process Audio Tap + ScreenCaptureKit — both are retained below as dated history at their commits. The **current release gate** (`docs/beta-release-checklist.md`, 2026-08-29 candidate) re-verifies only Serato, rekordbox, VirtualDJ, and the ScreenCaptureKit fallback, and still requires operator listening acceptance; Traktor and djay were **not** re-run against the current candidate. The matrix above reflects the current gate, so it does not carry Traktor/djay capture forward as currently verified. Neither statement overrides the other — they describe different dates and commits.
...
> **Naming note (2026-09-02):** the dated logs below predate the DJMemory→SetCatcher rename. Commands, the `DJMEMORY_FORCE_SCK_APP_AUDIO` env var, and the `DJMemory.app` bundle path are shown **as they actually ran under the former name** (then named DJMemory). Current equivalents: CLI `setcatcher` (`swift run setcatcher …`), app bundle `.build/SetCatcher.app/Contents/MacOS/SetCatcher`, env var `SETCATCHER_FORCE_SCK_APP_AUDIO`, entitlements file `packaging/SetCatcher.entitlements`.
...
- Probe update (2026-08-13, then named DJMemory): `swift run djmemory app-audio-probe <seconds> <softwareID>` and `.build/DJMemory.app/Contents/MacOS/DJMemory --app-audio-probe <seconds> <softwareID>` now archive successful metered captures through `ArchiveService.ingestCapture`, print archive + metadata paths, and confirm the new session appears in `SessionLibrary`.
...
- **Live verification complete (2026-08-14, then named DJMemory):** ran `swift run djmemory app-audio-probe 8 <id>` with real playback in each app.
  ...
  - M17 Process Audio Tap live verification was complete as of this 2026-08-14 run. **Superseded by the current release gate:** the 2026-08-29 candidate re-verifies only Serato, rekordbox, VirtualDJ, and the ScreenCaptureKit fallback, and still requires operator listening acceptance — see the reconciliation note under the App Matrix and `docs/beta-release-checklist.md`.
...
| M11b | App audio Capture + silence sessions | Implemented (all five — Serato, rekordbox, djay Pro 2, VirtualDJ, Traktor DJ 2 — showed meter + archive write on the 2026-08-12 probe. The current gate re-verifies only Serato, rekordbox, VirtualDJ + ScreenCaptureKit; Traktor/djay not re-run against the current candidate — see the reconciliation note under the App Matrix) |
