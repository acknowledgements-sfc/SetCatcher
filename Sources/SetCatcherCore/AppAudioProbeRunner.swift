...
/// Headless App audio Capture probe (shared by `setcatcher` CLI and `SetCatcherApp --app-audio-probe`).
public enum AppAudioProbeRunner {
    public static let defaultJSONLPath = "/tmp/setcatcher-invisible-capture-results.jsonl"
    ...
    public static func run(softwareID: String?, seconds: Int) {
        ...
        let jsonlPath = ProcessInfo.processInfo.environment["SETCATCHER_INVISIBLE_CAPTURE_JSONL"]
            ?? defaultJSONLPath
        let forceSCK = ProcessInfo.processInfo.environment["SETCATCHER_FORCE_SCK_APP_AUDIO"] == "1"
        let outputModeLabel = ProcessInfo.processInfo.environment["SETCATCHER_OUTPUT_MODE_LABEL"] ?? "UNKNOWN"
        ...
        Task {
            ...
            do {
                ...
                if !InvisibleCaptureProbeEvaluator.isExplicitOutputModeLabel(outputModeLabel) {
                    print("ERROR: set SETCATCHER_OUTPUT_MODE_LABEL to an explicit bench scenario (for example system-default or controller-usb). UNKNOWN cannot PASS.")
                    appendJSONL(
                        failedResult(
